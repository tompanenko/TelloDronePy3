...
def accepts(**types):
    def check_accepts(f):
        ...
        s = "accept number of arguments not equal with function number of arguments in ", fun_name, ", argcount ", argcount
        assert len(types) == argcount, s

        def new_f(*args, **kwds):
            for i, v in enumerate(args):
                if fun_code.co_varnames[i] in types and not isinstance(v, types[fun_code.co_varnames[i]]):
